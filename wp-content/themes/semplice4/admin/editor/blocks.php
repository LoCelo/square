		...
		foreach ($types as $type => $content) {
			...
			if($content['count'] > 0) {
				for($i = 1; $i <= $content['count']; $i++) {
					$blocks['list'][$type] .= '<li><a class="semplice-block" data-block-id="' . $type . '_' . $i . '" data-block-type="layout"><img src="' . get_template_directory_uri() . '/assets/images/admin/blocks/covers/' . $type . '_' . $i . '.png"></a></li>';
				}
			} else {
			...
			foreach($blocks as $block) {
				$output .= '
					<li id="block-' . $block->id . '">
						<a class="semplice-block editor-action" data-action-type="blocks" data-action="add" data-block-id="' . $block->id . '" data-block-type="user">
			...
			$output .= '
				<div class="no-blocks">
					<div class="inner">
						...
						<a class="blocks-help-video" href="https://www.semplice.com/videos#blocks" target="_blank"><img src="' . get_template_directory_uri() . '/assets/images/admin/no_blocks.png"></a>
					</div>
				</div>
			';
		}
